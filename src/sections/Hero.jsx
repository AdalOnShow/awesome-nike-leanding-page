    ...
    <section id="home" className="w-full flex flex-col xl:flex-row justify-center min-h-screen gap-10 max-container ">
      <div className="relative xl:w-2/5 flex flex-col justify-center items-start w-full max-xl:padding-x pt-28">
        ...
        <h1 className="mt-10 font-palanquin text-8xl max-sm:text[72px] max-sm:leading-[82px] font-bold">
          <span className="xl:bg-white xl:whitespace-nowrap relative z-[2] pr-10">The New Arrival</span>
          <br />
          ...
          {statistics.map((stat, i) => (
            <div key={i} className="">
              <p className="text-4xl font-palanquin font-bold">
                <CountUp end={stat.value} duration={4} />{stat.unit}
              </p>
        ...
        <div className="flex sm:gap-6 gap-4 absolute -bottom-[5%] sm:left-[10%] max-sm:px-6">
          {shoes.map((shoe, i) => (
            <div key={i} className="">
              <ShoeCard
                imgURL={shoe}
                changeBigShowImg={(shoe) => setBigShoeImg(shoe)}
